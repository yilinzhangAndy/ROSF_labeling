...
df = pd.read_excel('/Users/zhangyilin/Documents/UF/Ph.D/ROSF/ROSF_labeling_local/final_all_tasks.xlsx')
...
for index, row in df.iloc[start_task:end_task].iterrows():
    ...
    while not valid_input:
        ...
        for label_input in user_input.split(','):
            label_input = label_input.strip().lower()  # Trim whitespace and convert to lowercase
            if label_input in ['na', 'other']:
                label_data[label_input + ": " + labels[-2 if label_input == 'na' else -1].split(': ')[1]] = 1
            else:
                try:
                    label_index = int(label_input) - 1  # Convert to zero-based index
                    if 0 <= label_index < len(labels) - 2:  # Adjust for the 'na' and 'other' options
                        label_data[labels[label_index]] = 1
                    else:
                        print(f"Label number {label_input} is out of range. Please enter a valid label number.")
                        valid_input = False
                        break  # Exit the for loop, go back to the while loop for new input
                except ValueError:
                    print(f"Invalid input: {label_input}. Expected a number, 'na', or 'other'. Please enter a valid label number.")
                    valid_input = False
                    break  # Exit the for loop, go back to the while loop for new input
